...
/*
   Finally, we can put this all together to parse an entire HTML document into a DOM tree.
   This function will create a root node for the document if it doesn’t include one explicitly;
   this is similar to what a real HTML parser does.
*/
...
impl Parser {
    /*
       ...
       This function returns a Vec, which is Rust’s name for a growable array.
    */
    ...
    /*
        Now we’re ready to start parsing HTML.
        To parse a single node, we look at its first character to see if it is an element or a text node.
    ...
    */
    ...
    /*
       An element is more complicated.
       It includes opening and closing tags, and between them any number of child nodes:
    */
    ...
    /*
       Parsing attributes is pretty easy in our simplified syntax.
       Until we reach the end of the opening tag (>) we repeatedly look for a name followed by =
    ...
    */
    ...
    /*
       ...
       The consume_while method consumes characters that meet a given condition, and returns them as a string.
    ...
    */
    ...
    /*
       Rust strings are stored as UTF-8 byte arrays.
       To go to the next character, we can’t just advance by one byte.
       Instead we use char_indices which correctly handles multi-byte characters.
       (If our string used fixed-width characters, we could just increment pos by 1.)
    */
